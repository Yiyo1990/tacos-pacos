...
export class ModalGuisadoComponent implements OnInit{
  ...
  ngOnInit(): void {
    if(Object.keys(this.guisoEdit).length > 0) {
      console.log("editar",this.guisoEdit)
      this.editarGuisado()
    }
  }
  ...
  editarGuisado() {
    ...
    this.calComedorGordita.venta = comeGor.venta
    this.calComedorGordita.id = comeGor.id
    this.calAppTaco.venta = appTaco.venta
    this.calAppTaco.id = appTaco.id
    this.calAppGordita.venta = appGor.venta
    this.calAppGordita.id = appGor.id
